...
def make_emergency_stop(finger: wujihandpy.Finger):
...
def main() -> int:
    ...
    signal.signal(signal.SIGINT, make_emergency_stop(hand.finger(args.finger)))
    
    # Set effort limits: 1.5A for all joints on finger, 3.5A for the test joint
    cur = hand.read_joint_effort_limit()
    for j in range(4):  # All 4 joints on finger
        cur[args.finger, j] = 1.5
    cur[args.finger, args.joint] = 3.5  # Test joint gets higher limit
    hand.write_joint_effort_limit(cur)
    print(f"Set effort limit to 1.5 A for all joints on finger {args.finger}, 3.5 A for joint {args.joint}")
    
    # Enable all joints on the test finger
    hand.finger(args.finger).write_joint_enabled(True)
    print(f"Enabled all joints on finger {args.finger}")
    ...
    try:
        ...
        if csv_file:
            csv_file.close()
        try:
            # Disable all joints on the test finger
            hand.finger(args.finger).write_joint_enabled(False)
        except Exception:
            pass
